...
class Analyser:
    ...
    def doRanking(self,top=None):
        answers = open(self.afile, encoding='utf-8').read().lower()
        questions = open(self.qfile, encoding='utf-8').read().lower()

        # Define regex that will match the content of both text token or code token
        text_regex = "<t>(.+?)<\/t>"
        code_regex = "<c>(.+?)<\/c>"
        text_tokens_ans = re.findall(text_regex, answers)
        code_tokens_ans = re.findall(code_regex,answers)
        ...
        for i in joint.most_common(top):
            print(i)
